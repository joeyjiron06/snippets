...
function cleanupThreeState(container: HTMLDivElement, state: ThreeState | null) {
  if (!state) return
...
export default function PixelBlast({
  ...
  useEffect(() => {
    ...
    cleanupThreeState(container, threeRef.current)
  ...
  useEffect(() => {
    ...
    const liquidStrengthUniform = state.liquidEffect?.uniforms.get('uStrength')
    if (liquidStrengthUniform) liquidStrengthUniform.value = liquidStrength

    const liquidFreqUniform = state.liquidEffect?.uniforms.get('uFreq')
    if (liquidFreqUniform) liquidFreqUniform.value = liquidWobbleSpeed

    const noiseAmountUniform = state.noiseEffect?.uniforms.get('uAmount')
    if (noiseAmountUniform) noiseAmountUniform.value = noiseAmount
